...
export default function Home() {
  const [content, setContent] = useState(() => localStorage.getItem(STORAGE_KEY) || sample)
  ...
  return (
    <section aria-labelledby="main-heading" className="flex flex-row w-full h-[600px] gap-4">
      <h1 id="main-heading" className="sr-only">Markdown Editor</h1>

      <div className="flex-1 h-full overflow-auto flex flex-col justfy-center p1-6">
        ...
        <div className="mt-3 flex gap-2 flex-wrap">
          ...
          <input
            ...
            type="number"
            onKeyDown={(e) => { if (e.key === 'Enter') handleLoadFromApi(Number(e.target.value)) }}
            className="px-2 py-2 border rounded"
          />
        </div>

        <div className="mt-2 text-sm text-gray-600">Saved remote ID: {savedId ?? '—'}</div>
      </div>
